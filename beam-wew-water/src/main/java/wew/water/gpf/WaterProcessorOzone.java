...
public class WaterProcessorOzone {
    ...
    public static double O3excoeff(double wavelength) {
        ...
        return (interpolate(lambdas, extinctionCoefficients, wavelength));
    }

    private static double interpolate(double[] lambdas, double[] excoeff, double wavelength) {
        for (int i = 0; i < lambdas.length - 1; i++) {
            final double lambda1 = lambdas[i];
            final double lambda2 = lambdas[i + 1];
            if (wavelength >= lambda1 && wavelength <= lambda2) {
                final double excoeff1 = excoeff[i];
                final double excoeff2 = excoeff[i + 1];
                return (excoeff1 + (wavelength - lambda1) * (excoeff2 - excoeff1) / (lambda2 - lambda1));
            }
        }
        return Double.NaN;
    }
