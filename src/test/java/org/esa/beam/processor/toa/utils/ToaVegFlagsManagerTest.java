/*
 * $Id: ToaVegFlagsManagerTest.java,v 1.3 2005/11/03 10:32:35 meris Exp $
 *
...
public class ToaVegFlagsManagerTest extends TestCase {

    public ToaVegFlagsManagerTest(String name) {
        super(name);
    }

    public static Test suite() {
        return new TestSuite(ToaVegFlagsManagerTest.class);
    }
    ...
    /**
     * Tests all flag descriptions for correctness
     */
    public void testFlagDescriptions() {
        assertEquals("The pixel is invalid. This flag is a combination of INVALID_INPUT and ALGORITHM_FAILURE.", VegFlagsManager.INVALID_FLAG_DESCRIPTION);
        assertEquals("Set 'true' if any one of the input measurement data has a value <= 0.0 or if the input pixel was skipped due to input flagging.", VegFlagsManager.INVALID_INPUT_FLAG_DESCRIPTION);
        assertEquals("Set 'true' on any numerical failure during the processing (division by zero ?).", VegFlagsManager.ALGORITHM_FAILURE_FLAG_DESCRIPTION);
        assertEquals("Set 'true' if the calculated fraction of vegetation is out of the valid range.", VegFlagsManager.FCOVER_OUT_OF_RANGE_FLAG_DESCRIPTION);
        assertEquals("Set 'true' if the calculated leaf area index for this pixel is out of the valid range.", VegFlagsManager.LAI_OUT_OF_RANGE_FLAG_DESCRIPTION);
        assertEquals("Set 'true' if the calculated canopy chlorophyll content is out of the valid range.", VegFlagsManager.LAIXCAB_OUT_OF_RANGE_FLAG_DESCRIPTION);
        assertEquals("Set 'true' if the calculated Fraction of Absorbed Photosynthetically Active Radiation is out of the valid range.", VegFlagsManager.FAPAR_OUT_OF_RANGE_FLAG_DESCRIPTION);
    }
    ...
    public void testFlagCodings() {
        ...
        coding = VegFlagsManager.getCoding(ToaVegConstants.VEG_FLAGS_BAND_NAME);
        assertNotNull(coding);

        assertEquals(ToaVegConstants.VEG_FLAGS_BAND_NAME, coding.getName());
