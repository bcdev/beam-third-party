/*
 * $Id: TocVegRequestElementFactoryTest.java,v 1.6 2006/03/27 15:33:02 meris Exp $
 *
...
public class TocVegRequestElementFactoryTest extends TestCase {

    private VegRequestElementFactory _factory;
    private URL _url;
    private File _file;

    public TocVegRequestElementFactoryTest(String name) {
        super(name);
    }

    public static Test suite() {
        return new TestSuite(TocVegRequestElementFactoryTest.class);
    }
    ...
    protected void setUp() {
        _factory = VegRequestElementFactory.getTocVegInstance();
    ...
    public void testSingletonInterface() {
        ...
        ref_1 = VegRequestElementFactory.getTocVegInstance();
        assertNotNull(ref_1);

        // must return something
        ref_2 = VegRequestElementFactory.getTocVegInstance();
        assertNotNull(ref_2);
    ...
    public void testBitmaskParameter() {
        ...
        try {
            param = _factory.createParameter(TocVegConstants.BITMASK_PARAM_NAME, correctVal);
            // we must get a parameter
            assertNotNull(param);
            // name must fit
            assertEquals(TocVegConstants.BITMASK_PARAM_NAME, param.getName());
            ...
            // label must fit
            assertEquals(TocVegConstants.BITMASK_PARAM_LABEL,
                         param.getProperties().getLabel());
            // description must fit
            assertEquals(TocVegConstants.BITMASK_PARAM_DESCRIPTION,
                         param.getProperties().getDescription());
            // default value must fit
            assertEquals(TocVegConstants.BITMASK_PARAM_DEFAULT,
